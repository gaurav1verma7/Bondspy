...
def basicPageSetup(pageNo):
    ...
    heading = tk.Label(pages[pageNo], fg="white",
                       bg="#3E3B3C", font="Arial 20 bold", pady=10)
    heading.pack()
...
def getNewSlide(control):
    ...
    if (len(img_list) > 1):
        ...
        slide_caption.configure(text="Image {} of {}".format(
            current_slide+1, len(img_list)))
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if (len(path_list) < 5):
        ...
        for i in range(len(img_list)):
            ch = "Image " + str(i+1)
            opt_menu['menu'].add_command(
                label=ch, command=tk._setit(menu_var, ch))
...
def register(entries, required, menu_var):
    ...
    if (len(no_face) > 0):
        ...
        if (rowId >= 0):
            ...
            cv2.imwrite("profile_pics/criminal %d.png" %
                        rowId, img_list[profile_img_num])
...
def getPage1():
    ...
    canvas.bind('<Configure>', lambda event, canvas=canvas,
                win=scroll_win: on_configure(event, canvas, win))

    tk.Label(scroll_frame, text="* Required Fields", bg="#3E3B3C",
             fg="yellow", font="Arial 13 bold").pack()
    # Adding Input Fields
    input_fields = ("Name", "Father's Name", "Gender",
                    "DOB(yyyy-mm-dd)", "Crimes Done", "Profile Image")
    ...
    for i, field in enumerate(input_fields):
        ...
        label = tk.Text(row, width=20, height=1, bg="#3E3B3C",
                        fg="#ffffff", font="Arial 13", highlightthickness=0, bd=0)
        ...
        if (i != ip_len-1):
            ...
            opt_menu.configure(font="Arial 13", bg="#000000", fg="white",
                               bd=0, highlightthickness=0, activebackground="#3E3B3C")
...
def showCriminalProfile(name):
    ...
    for i, item in enumerate(crim_data.items()):
        tk.Label(info_frame, text=item[0], pady=15, fg="yellow", font="Arial 15 bold", bg="#202d42").grid(
        ...
        tk.Label(info_frame, text=val.capitalize(), fg="white",
                 font="Arial 15", bg="#202d42").grid(row=i, column=2, sticky='w')
...
def selectImage():
    ...
    path = filedialog.askopenfilename(
        title="Choose a image", filetypes=filetype)
...
def selectvideo():
    ...
    path = filedialog.askopenfilename(
        title="Choose a video", filetypes=filetype)
    p = ''
    p = path
